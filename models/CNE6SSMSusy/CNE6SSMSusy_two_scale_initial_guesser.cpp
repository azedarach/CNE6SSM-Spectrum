...
CNE6SSMSusy_initial_guesser<Two_scale>::CNE6SSMSusy_initial_guesser(
   CNE6SSMSusy<Two_scale>* model_,
   const QedQcd& oneset_,
   const CNE6SSMSusy_low_scale_constraint<Two_scale>& low_constraint_,
   const CNE6SSMSusy_high_scale_constraint<Two_scale>& high_constraint_
)
...
{
   assert(model && "CNE6SSMSusy_initial_guesser: Error: pointer to model"
          " CNE6SSMSusy<Two_scale> must not be zero");
}
 ...
 * guess_low_scale_parameters() and guess_high_scale_parameters() .
 */
void CNE6SSMSusy_initial_guesser<Two_scale>::guess()
{
...
/**
 * Guesses the low scale SUSY parameters (gauge, Yukawa couplings) at
 ...
 * \code{.cpp}
   ...
   const auto ssumInput = INPUTPARAMETER(sInput);
   ...
   MODEL->set_vs(sInput * Cos(ArcTan(TanTheta)));
   MODEL->set_vsb(sInput * Sin(ArcTan(TanTheta)));
   MODEL->set_vphi(vphiInput);
   MODEL->set_QS(QSInput);

 * \endcode
 */
void CNE6SSMSusy_initial_guesser<Two_scale>::guess_low_scale_parameters()
{
...
void CNE6SSMSusy_initial_guesser<Two_scale>::calculate_DRbar_yukawa_couplings()
{
   calculate_Yu_DRbar();
   calculate_Yd_DRbar();
   calculate_Ye_DRbar();
 ...
 * corrections).
 */
void CNE6SSMSusy_initial_guesser<Two_scale>::calculate_Yu_DRbar()
{
 ...
 */
void CNE6SSMSusy_initial_guesser<Two_scale>::calculate_Yd_DRbar()
{
 ...
 * corrections).
 */
void CNE6SSMSusy_initial_guesser<Two_scale>::calculate_Ye_DRbar()
{
...
/**
 * Guesses the high-scale SUSY parameters.  At first it runs to the
 ...
 */
void CNE6SSMSusy_initial_guesser<Two_scale>::guess_high_scale_parameters()
{
