...
char const * const lowNE6SSM_slha_io::drbar_blocks[NUMBER_OF_DRBAR_BLOCKS] =
...
lowNE6SSM_slha_io::lowNE6SSM_slha_io()
   : slha_io()
{
}

void lowNE6SSM_slha_io::clear()
{
   slha_io.clear();
}

void lowNE6SSM_slha_io::set_extpar(const lowNE6SSM_input_parameters& input)
{
   ...
   extpar << FORMAT_ELEMENT(73, input.TanTheta, "TanTheta");
   slha_io.set_block(extpar);

}

void lowNE6SSM_slha_io::set_minpar(const lowNE6SSM_input_parameters& input)
{
...
void lowNE6SSM_slha_io::set_sminputs(const softsusy::QedQcd& qedqcd)
{
   slha_io.set_sminputs(qedqcd);
}

void lowNE6SSM_slha_io::set_spinfo(const Problems<lowNE6SSM_info::NUMBER_OF_PARTICLES>& problems)
{
...
void lowNE6SSM_slha_io::write_to_file(const std::string& file_name)
{
   slha_io.write_to_file(file_name);
}

double lowNE6SSM_slha_io::get_input_scale() const
{
   return slha_io.get_extpar().input_scale;
}

double lowNE6SSM_slha_io::get_parameter_output_scale() const
{
   return slha_io.get_modsel().parameter_output_scale;
}

void lowNE6SSM_slha_io::read_from_file(const std::string& file_name)
{
...
void lowNE6SSM_slha_io::fill(lowNE6SSM_input_parameters& input) const
{
   SLHA_io::Tuple_processor minpar_processor
      = boost::bind(&lowNE6SSM_slha_io::fill_minpar_tuple, boost::ref(input), _1, _2);
   SLHA_io::Tuple_processor extpar_processor
      = boost::bind(&lowNE6SSM_slha_io::fill_extpar_tuple, boost::ref(input), _1, _2);
...
void lowNE6SSM_slha_io::fill(Spectrum_generator_settings& settings) const
{
   SLHA_io::Tuple_processor flexiblesusy_processor
      = boost::bind(&lowNE6SSM_slha_io::fill_flexiblesusy_tuple, boost::ref(settings), _1, _2);

   slha_io.read_block("FlexibleSUSY", flexiblesusy_processor);
}

void lowNE6SSM_slha_io::fill_minpar_tuple(lowNE6SSM_input_parameters& input,
...
void lowNE6SSM_slha_io::fill_extpar_tuple(lowNE6SSM_input_parameters& input,
                                                int key, double value)
{
   ...
   case 73: input.TanTheta = value; break;
   default: WARNING("Unrecognized key: " << key); break;
   }

}

void lowNE6SSM_slha_io::fill_flexiblesusy_tuple(Spectrum_generator_settings& settings,
                                                  int key, double value)
{
 ...
 */
double lowNE6SSM_slha_io::read_scale() const
{
   double scale = 0.;
