...
#ifndef lowNE6SSM_TWO_SCALE_INITIAL_GUESSER_H
#define lowNE6SSM_TWO_SCALE_INITIAL_GUESSER_H

#include "lowNE6SSM_initial_guesser.hpp"
...
/**
 * @class lowNE6SSM_initial_guesser<Two_scale>
 * @brief initial guesser for the lowNE6SSM
 */

template<>
class lowNE6SSM_initial_guesser<Two_scale> : public Initial_guesser<Two_scale> {
public:
   lowNE6SSM_initial_guesser(lowNE6SSM<Two_scale>*,
                               const lowNE6SSM_input_parameters&,
                               const QedQcd&,
                               const lowNE6SSM_low_scale_constraint<Two_scale>&,
                               const lowNE6SSM_susy_scale_constraint<Two_scale>&);
   virtual ~lowNE6SSM_initial_guesser();
   virtual void guess(); ///< initial guess

   void set_running_precision(double p) { running_precision = p; }

private:
   lowNE6SSM<Two_scale>* model; ///< pointer to model class
   lowNE6SSM_input_parameters input_pars;
   ...
   lowNE6SSM_low_scale_constraint<Two_scale> low_constraint;
   lowNE6SSM_susy_scale_constraint<Two_scale> susy_constraint;
