...
#include "lowNE6SSM_spectrum_generator.hpp"
...
int main()
{
   ...
   lowNE6SSM_spectrum_generator<algorithm_type> spectrum_generator;
   ...
   for (std::vector<double>::const_iterator it = range.begin(),
      ...
      const lowNE6SSM<algorithm_type>& model = spectrum_generator.get_model();
      const lowNE6SSM_physical& pole_masses = model.get_physical();
      const Problems<lowNE6SSM_info::NUMBER_OF_PARTICLES>& problems
