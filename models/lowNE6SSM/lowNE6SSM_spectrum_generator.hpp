...
// File generated at Sun 21 Sep 2014 21:56:26

#ifndef lowNE6SSM_SPECTRUM_GENERATOR_H
#define lowNE6SSM_SPECTRUM_GENERATOR_H
...
#include "lowNE6SSM_two_scale_convergence_tester.hpp"
#include "lowNE6SSM_two_scale_initial_guesser.hpp"
#include "lowNE6SSM_utilities.hpp"
...
template <class T>
class lowNE6SSM_spectrum_generator {
public:
   lowNE6SSM_spectrum_generator()
      : solver(), model()
   ...
   const lowNE6SSM<T>& get_model() const { return model; }
   const Problems<lowNE6SSM_info::NUMBER_OF_PARTICLES>& get_problems() const {
   ...
   void run(const QedQcd& oneset, const lowNE6SSM_input_parameters& input);
   void write_running_couplings(const std::string& filename = "lowNE6SSM_rge_running.dat") const;
   void write_spectrum(const std::string& filename = "lowNE6SSM_spectrum.dat") const;

private:
   RGFlow<T> solver;
   lowNE6SSM<T> model;
   lowNE6SSM_susy_scale_constraint<T> susy_scale_constraint;
   lowNE6SSM_low_scale_constraint<T>  low_scale_constraint;
 ...
 */
template <class T>
void lowNE6SSM_spectrum_generator<T>::run(const QedQcd& oneset,
                                const lowNE6SSM_input_parameters& input)
{
   ...
   lowNE6SSM_convergence_tester<T> convergence_tester(&model, precision_goal);
   if (max_iterations > 0)
      convergence_tester.set_max_iterations(max_iterations);

   lowNE6SSM_initial_guesser<T> initial_guesser(&model, input, oneset,
                                                  low_scale_constraint,
                                                  susy_scale_constraint);
 ...
 */
template <class T>
void lowNE6SSM_spectrum_generator<T>::write_running_couplings(const std::string& filename) const
{
   lowNE6SSM<T> tmp_model(model);
   tmp_model.run_to(low_scale);

   lowNE6SSM_parameter_getter parameter_getter;
   Coupling_monitor<lowNE6SSM<T>, lowNE6SSM_parameter_getter>
 ...
 */
template <class T>
void lowNE6SSM_spectrum_generator<T>::write_spectrum(const std::string& filename) const
{
   lowNE6SSM_spectrum_plotter plotter;
