...
template <class T>
class CNE6SSM_spectrum_generator {
public:
   ...
   const CNE6SSM<T>& get_model() const { return model; }
                                ...
                                const CNE6SSM_input_parameters& input)
{
   ...
   CNE6SSM_initial_guesser<T> initial_guesser(&model, input, oneset,
                                                  low_scale_constraint,
                                                  susy_scale_constraint,
                                                  high_scale_constraint);
 ...
 */
template <class T>
void CNE6SSM_spectrum_generator<T>::write_running_couplings(const std::string& filename) const
{
   CNE6SSM<T> tmp_model(model);
   tmp_model.run_to(low_scale);

   CNE6SSM_parameter_getter parameter_getter;
   Coupling_monitor<CNE6SSM<T>, CNE6SSM_parameter_getter>
