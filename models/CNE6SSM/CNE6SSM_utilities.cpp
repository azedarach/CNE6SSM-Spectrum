...
void CNE6SSM_spectrum_plotter::write_to_file(const std::string& file_name) const
{
   if (spectrum.empty())
      return;

   std::ofstream filestr(file_name.c_str(), std::ios::out);
   VERBOSE_MSG("CNE6SSM_spectrum_plotter::write_to_file: opening file: "
               << file_name.c_str());
   if (filestr.fail()) {
      ERROR("CNE6SSM_spectrum_plotter::write_to_file: can't open file "
            << file_name);
      return;
   }
   ...
   filestr.close();
   VERBOSE_MSG("CNE6SSM_spectrum_plotter::write_to_file: file written: "
               << file_name.c_str());
}

void CNE6SSM_spectrum_plotter::extract_spectrum(const CNE6SSM_mass_eigenstates& model)
{
   spectrum.clear();
...
void CNE6SSM_spectrum_plotter::write_spectrum(const TSpectrum& spectrum, std::ofstream& filestr) const
{
   for (std::size_t s = 0; s < spectrum.size(); ++s) {
      if (!filestr.good()) {
         ERROR("CNE6SSM_spectrum_plotter::write_spectrum: "
...
std::valarray<double> CNE6SSM_spectrum_plotter::to_valarray(double v)
{
