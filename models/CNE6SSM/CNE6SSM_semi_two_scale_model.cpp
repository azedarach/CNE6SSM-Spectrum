...
/**
 * @file CNE6SSM_semi_two_scale_model.cpp
...
CLASSNAME::CNE6SSM_semianalytic(const CNE6SSM_semianalytic_input_parameters<Two_scale>& input_)
   : Two_scale_model()
...
CLASSNAME::~CNE6SSM_semianalytic()
{
}
...
)
{
   ...
   const bool is_finite = IsFinite(LXiF);
...
void CLASSNAME::tree_level_ewsb_initial_guess(double x_init[number_of_tree_level_ewsb_eqs])
{
   ...
   x_init[0] = (mHd2_m122_coeff * Sqr(oneOTanBeta) - mHu2_m122_coeff) * Sqr(m12)
      ...
      + 0.0125 * Sqr(g1p) * LOCALINPUT(QSInput) * Sqr(s) * Sqr(Sigmax) *
 ...
 * GSL_EDOM otherwise.
 */
int CLASSNAME::ewsb_step(double ewsb_parameters[number_of_tadpole_equations])
{
   ...
   double m0 = (mHd2_m122_coeff * Sqr(vd) - mHu2_m122_coeff * Sqr(vu)) * Sqr(m12)
      ...
      + 0.5 * Sqr(Lambdax) * Sqr(vs) * (Sqr(vd) - Sqr(vu)) +
...
int CLASSNAME::ewsb_step(const gsl_vector* x, void* params, gsl_vector* f)
{
   ...
   double ewsb_parameters[number_of_tadpole_equations] =
...
std::ostream& operator<<(std::ostream& ostr, const CNE6SSM_semianalytic<Two_scale>& model)
{
   model.print(ostr);
   return ostr;
}
