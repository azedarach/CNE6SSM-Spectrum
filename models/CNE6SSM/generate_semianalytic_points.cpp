...
void set_default_parameter_values(CNE6SSM_semianalytic_input_parameters<Two_scale>& input)
{
   input.m12 = 3500.;
   input.Azero = 8000.;
   input.TanBeta = 10.;
...
void write_comment_line(std::ostream & out)
{
   ...
   out << "# ";
   write_CNE6SSM_semianalytic_inputs_list(out, width);
   out << '\n';
}

void write_point(const CNE6SSM_semianalytic_input_parameters<Two_scale>& input,
                 std::ostream & out)
{
   const std::size_t width = 18;

   write_CNE6SSM_inputs(input, out, width);
   out << '\n';
}
