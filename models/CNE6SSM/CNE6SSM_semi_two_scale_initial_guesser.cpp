...
#include "CNE6SSM_susy_two_scale_convergence_tester.hpp"
#include "lowe.h"
#include "error.hpp"
...
CNE6SSM_semianalytic_initial_guesser<Two_scale>::CNE6SSM_semianalytic_initial_guesser(
   CNE6SSM_semianalytic<Two_scale>* model_,
   const QedQcd& oneset_,
   const CNE6SSM_semianalytic_low_scale_constraint<Two_scale>& low_constraint_,
   const CNE6SSM_semianalytic_susy_scale_constraint<Two_scale>& susy_constraint_,
   const CNE6SSM_semianalytic_high_scale_constraint<Two_scale>& high_constraint_,
   CNE6SSM_semianalytic_high_scale_constraint<Two_scale>* main_iteration_high_constraint_
)
...
{
   assert(model && "CNE6SSM_semianalytic_initial_guesser: Error: pointer to model"
          " CNE6SSM_semianalytic<Two_scale> must not be zero");
}
 ...
 */
void CNE6SSM_semianalytic_initial_guesser<Two_scale>::guess()
{
 ...
 */
void CNE6SSM_semianalytic_initial_guesser<Two_scale>::guess_susy_parameters()
{
   ...
   CNE6SSM_susy_convergence_tester<Two_scale> convergence_tester(model, running_precision);
...
void CNE6SSM_semianalytic_initial_guesser<Two_scale>::guess_low_scale_parameters(CNE6SSM_semianalytic<Two_scale>* model, const QedQcd& oneset, double g1, double g2, double g3)
{
   const auto TanBeta = INPUTPARAMETER(TanBeta);
   const auto QSInput = INPUTPARAMETER(QSInput);
   const auto sInput = INPUTPARAMETER(sInput);
...
void CNE6SSM_semianalytic_initial_guesser<Two_scale>::guess_high_scale_parameters()
{
...
void CNE6SSM_semianalytic_initial_guesser<Two_scale>::calculate_DRbar_yukawa_couplings()
{
   calculate_Yu_DRbar();
   calculate_Yd_DRbar();
   calculate_Ye_DRbar();
 ...
 * calculated.
 */
void CNE6SSM_semianalytic_initial_guesser<Two_scale>::guess_soft_parameters()
{
   const double low_scale_guess = low_constraint.get_initial_scale_guess();
...
void CNE6SSM_semianalytic_initial_guesser<Two_scale>::Initial_guess_low_scale_constraint::set_model(Two_scale_model* model_)
{
