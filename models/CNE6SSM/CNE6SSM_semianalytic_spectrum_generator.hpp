...
#ifndef CNE6SSM_SEMIANALYTIC_SPECTRUM_GENERATOR_H
#define CNE6SSM_SEMIANALYTIC_SPECTRUM_GENERATOR_H

#include "CNE6SSM_semi_two_scale_model.hpp"
#include "CNE6SSM_semi_two_scale_convergence_tester.hpp"
#include "CNE6SSM_semi_two_scale_initial_guesser.hpp"
#include "CNE6SSM_semi_two_scale_constraint_handler.hpp"
...
template <class T>
class CNE6SSM_semianalytic_spectrum_generator {
public:
   CNE6SSM_semianalytic_spectrum_generator()
      : solver(), model()
      , constraint_handler()
      , high_scale(0.)
      , susy_scale(0.)
   ...
   const CNE6SSM_semianalytic<T>& get_model() const { return model; }
   ...
   void run(const QedQcd& oneset, const CNE6SSM_semianalytic_input_parameters<T>& input);
...
private:
   ...
   CNE6SSM_semianalytic<T> model;
   CNE6SSM_semianalytic_constraint_handler<T> constraint_handler;
 ...
 */
template <class T>
void CNE6SSM_semianalytic_spectrum_generator<T>::run(const QedQcd& oneset,
                                                     const CNE6SSM_semianalytic_input_parameters<T>& input)
{
   model.clear();
   ...
   CNE6SSM_susy_convergence_tester<T> inner_convergence_tester(&model, precision_goal);

   // convergence tester for the whole iteration
   CNE6SSM_semianalytic_convergence_tester<T> outer_convergence_tester(&model, precision_goal);
   ...
   CNE6SSM_semianalytic_initial_guesser<T> initial_guesser(constraint_handler.get_initial_guesser(&model, oneset));
 ...
 */
template <class T>
void CNE6SSM_semianalytic_spectrum_generator<T>::write_running_couplings(const std::string& filename) const
{
   CNE6SSM_semianalytic<T> tmp_model(model);
   tmp_model.run_to(low_scale);

   CNE6SSM_parameter_getter parameter_getter;
   Coupling_monitor<CNE6SSM_semianalytic<T>, CNE6SSM_parameter_getter>
 ...
 */
template <class T>
void CNE6SSM_semianalytic_spectrum_generator<T>::write_spectrum(const std::string& filename) const
{
